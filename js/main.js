...
const url = `https://fakestoreapi.com/products`
 ...
 // Getting the Categories         
     ...
     data.forEach(obj => { 
         ...
         all_rate = 0;
         data.forEach(item => {
            rate_array.push(item.rating.rate)
            console.log(rate_array)
            all_ratings += item.rating.count;
            all_rate += item.rating.count * item.rating.rate
    ...
    // Getting the Products
            ...
            //   <span>${item.rating.rate}</span>
              articles.innerHTML = `
                ...
                <div class="article_star-one">
                    <div class="article_star-two"></div><i class="fa-solid fa-chevron-down"></i>
                </div>
                <span class= "article_count"> ${item.rating.count.toLocaleString()}</span>
                </div>
          ...
          //Setting each product ratings 
          let average_stars = (all_rate / all_ratings).toFixed(1)
          let star_article = document.querySelectorAll('.article_star-two')
          star_article.forEach(star => {
            star.style.width = (average_stars / 5) * 100 + "%";
          })
